<template>
 ...
 >
  ...
  >
   ...
   <div class="flex-1">
    <span class="flex items-center bg-search border rounded-xl px-5 relative">
     <InputText
      v-model="searchQuery"
   ...
   <div>
    <ul class="flex gap-5">
     ...
     <nuxt-link to="/wishList" class="flex gap-1">
      <i class="pi pi-heart" style="font-size: 1.3rem; color: #478c5c"></i>
     </nuxt-link>
     <button
      class="flex gap-1 cursor-pointer"
  ...
  >
   <ul class="flex flex-col gap-2 border-r px-5 relative">
    ...
    >
     ...
     <ul
      v-if="isSubMenuVibile && index === activeIndex"
...
export default {
 setup() {
  ...
  function showSubMenu(index) {
   isSubMenuVibile = !isSubMenuVibile;
   activeIndex = index;
  }

  const { items, searchData } = storeToRefs(allItemsStore);
  const {
   isSidebarVisible,
   isSignInOpen,
   isSearchVisible,
